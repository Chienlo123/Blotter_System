...
if(isset($_POST['btn_add'])){
    ...
    $date = date('Y-m-d');
	 ...
	 if(isset($_SESSION['role'])){
        $action = 'Added file Blotter Request by '.$txt_headchief;
        $iquery = mysqli_query($con,"INSERT INTO tbllogs (user,logdate,action) values ('".$_SESSION['role']."', NOW(), '".$action."')");
    }

    if($_SESSION['role'] == "Administrators")
    {
    $query = mysqli_query($con,"INSERT INTO tblblotter (residentid,yearRecorded,dateRecorded,complainant,cage,caddress,ccontact,personToComplain,compgen,paddress,headchief,complaint,respgen,sStatus,locationOfIncidence,recordedby,status) 
...
if(isset($_POST['btn_req'])){
   $txt_cname = $_POST['txt_cname'];
    ...
    $reqquery = mysqli_query($con,"INSERT INTO tblblotter (residentid,yearRecorded,dateRecorded,complainant,cage,caddress,ccontact,personToComplain,compgen,paddress,headchief,complaint,respgen,sStatus,locationOfIncidence,recordedby,status) 
...
if(isset($_POST['btn_approve']))
{
    ...
    $disapprove_query = mysqli_query($con,"UPDATE tblblotter set status = 'Approved'  where id = '".$txt_id."' ") or die('Error: ' . mysqli_error($con));
      

	  if(isset($_SESSION['role'])){
        $action = 'Approved file Blotter Request by user';
        $iquery = mysqli_query($con,"INSERT INTO tbllogs (user,logdate,action) values ('".$_SESSION['role']."', NOW(), '".$action."')");
          }	
...
if(isset($_POST['btn_save']))
{
   ...
   $txt_id = $_POST['hidden_id'];
    ...
    if(isset($_SESSION['role'])){
        $action = 'Edit and Update File Blotter Request by '.$txt_edit_cname;
        $iquery = mysqli_query($con,"INSERT INTO tbllogs (user,logdate,action) values ('".$_SESSION['role']."', NOW(), '".$action."')");
    }
